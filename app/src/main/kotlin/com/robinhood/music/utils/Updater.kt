...
object Updater {
    ...
    private const val GITHUB_API_BASE = "https://api.github.com/repos/Faroffcode/robinHood"
    ...
    private fun parseAssets(assetsArray: JSONArray): List<ReleaseAsset> {
        ...
        for (i in 0 until assetsArray.length()) {
            ...
            val (arch, variant) = when {
                name == "RobinHood.apk" -> "universal" to "universal"
    ...
    fun getDownloadUrlForCurrentVariant(releaseInfo: ReleaseInfo): String? {
        ...
        return (
            releaseInfo.assets.find { it.architecture == currentArch && it.variant == currentVariant }
                ?: releaseInfo.assets.find { it.name == "RobinHood.apk" }
        )?.downloadUrl
    }
